...
from utils.factor.basic.calculator import AlphaCalculator, TensorAlphaCalculator
from utils.factor.basic.expression import Expression
from utils.factor.basic.calculator import OutOfDataRangeError
from utils.factor.basic.pool_update import PoolUpdate, AddRemoveAlphas
from utils.factor.basic.correlation import batch_pearsonr
...
class LinearAlphaPool(AlphaPoolBase, metaclass=ABCMeta):
    def __init__(
        ...
        self._ic_lower_bound = -1.0 if ic_lower_bound is None else ic_lower_bound
        self.best_obj = -1.0
    ...
    def force_load_exprs(
        self, exprs: List[Expression], weights: Optional[List[float]] = None
    ) -> None:
        self._failure_cache = set()
        ...
        if weights is not None:
            if len(weights) != self.size:
                raise ValueError(
                    f"Invalid weights length: got {len(weights)}, expected {self.size}"
                )
    ...
    @abstractmethod
    def optimize(
        self, lr: float = 5e-4, max_steps: int = 10000, tolerance: int = 500
    ) -> np.ndarray:
    ...
    def _pop(self, index_hint: Optional[int] = None) -> None:
        if self.size <= self.capacity:
            return
        index = (
            int(np.argmin(np.abs(self.weights))) if index_hint is None else index_hint
        )
    ...
    def bulk_edit(
        self, removed_indices: Iterable[int], added_exprs: List[Expression]
    ) -> None:
        self._failure_cache = set()
        old_ic = self.evaluate_ensemble()
        old_pool: List[Expression] = self.exprs[: self.size]  # type: ignore
        ...
        removed_indices = list(
            sorted(i for eid, i in old_exprs.items() if eid not in new_exprs)
        )
        new_ic, new_obj = self.calculate_ic_and_objective()
        self._maybe_update_best(new_ic, new_obj)
        self.update_history.append(
...
class MseAlphaPool(LinearAlphaPool):
    ...
    def optimize(
        ...
        if math.isclose(
            alpha, 0.0
        ):  # No L1 regularization, use the faster least-squares method
    ...
    def _optimize_lstsq(self) -> np.ndarray:
        try:
            return np.linalg.lstsq(
                self._mutual_ics[: self.size, : self.size], self.single_ics[: self.size]
            )[0]
...
class MeanStdAlphaPool(LinearAlphaPool):
    ...
    def _calc_obj_impl(
        self, alpha_values: torch.Tensor, weights: torch.Tensor
    ) -> torch.Tensor:
